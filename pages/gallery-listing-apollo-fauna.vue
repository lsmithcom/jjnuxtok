<template>
    ...
    <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
      ...
      <div v-for="gallery in gallery.allGallery.data">
      <div class="items-center gap-8">
      <NuxtLink :to="`/gallery/` + gallery._id"><img :src="`/image/` + gallery.image" :alt="gallery.imageAlt" class="">
      <div class="block clear-both">
        <p class="text-2xl">{{gallery.title}}</p>
        <p class="text-lg my-3">{{gallery.description}}</p>   
      </div></NuxtLink>
    </div>
  </div>
        </div>
...
`
const { data: gallery } = await useAsyncQuery(query)
</script>