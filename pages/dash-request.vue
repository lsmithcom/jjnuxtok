...
<div class="ml-8 font-bold text-2xl text-agrey">Lead<span class="font-medium ml-4 pl-4 text-xl border-l border-agrey"><NuxtLink to="/dash-marketing">Marketing</NuxtLink></span></div>
<div class="flex flex-row my-0 items-end mr-8"><NuxtLink class="dbttn jbttn-dash" to="/marketing">New Lead</NuxtLink></div>
...
<div class="grid grid-cols-7 text-center items-center text-white bg-jblucrn text-sm font-semibold border-r border-white py-3 grid-7-a">
    ...
    <div class="border-r border-white">Company</div>
    <div class="border-r border-white">Phone</div>
    <div class="border-r border-white">Email</div>
    <div class="border-r border-white">Date</div> 
  </div>

  <div class="grid grid-cols-7 text-center items-center gridded text-agrey text-base grid-7-a" v-for="request in requests.allRequest.data">
     <DashRequest :request="request" />
  </div>  
...
`
const { data: requests } = await useAsyncQuery(query)
</script>